...
我定义了两个256bit长的float向量并分别初始化为 102.4 和 1.024，定义了一个256bit长的float向量来存储float向量加法的结果，然后打印出来。
...
我使用随机数填充了两个有着一千万元素的float数组，先用串行循环算出两个数组的和，算出时间，再用avx指令计算。在使用avx的时候，用到了
