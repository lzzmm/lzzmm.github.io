...
通过NVDIA的矩阵计算函数库 CUBLAS 计算矩阵相乘，矩阵规模从512增加至8192。
...
```c
CONV2D<<<gridSize, blockSize, 0>>>(dInMat, dKernel, dOutMat, in2DSize, out2DSize, MATRIX_CHANNEL, FILTER_NUM, FILTER_SIZE, STRIDE, zeroPadding);
```
