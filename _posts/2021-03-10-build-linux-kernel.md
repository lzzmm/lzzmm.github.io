...
3. 进入 `settings`，然后在 `Build static binary(no shared libs)` 处输入 `Y` 勾选，然后分别设置 `() Additional CFLAGS` 和 `() Additional LDFLAGS` 为 `(-m32 -march=i386) Additional CFLAGS` 和 `(-m32) Additional LDFLAGS` ，保存退出。![2021-03-07 02-09-37 的屏幕截图](/img/in-post/os/2021-03-07 02-09-37 的屏幕截图.png)
